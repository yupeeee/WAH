...
class ClassificationDataset(Dataset):
    ...
    def _check(
        self, checklist: List[Tuple[os.PathLike, str]], root: os.PathLike
    ) -> bool:
    ...
    def _download(
        ...
        ### Args
            ...
            - `checklist` (List[Tuple[os.PathLike, str]]): List of (filepath, checksum) tuples to verify. Should include both downloaded and extracted files.
            - `extract_dir` (Optional[os.PathLike]): Directory to extract downloaded files to, relative to self.root. Defaults to "".
        ...
        dataset_exists = os.path.exists(dataset_root) and all(
            os.path.exists(os.path.join(dataset_root, path))
            for path, _ in checklist[len(urls) :]
        )
        # If so, return
        if dataset_exists and self._check(checklist[len(urls) :], self.root):
            return
        # Else, download and verify files
        while True:
            # Download files
            paths = [download.from_url(url, self.root) for url in urls]
            # Verify downloads
            if self._check(checklist[: len(urls)], self.root):
                break
        ...
        for path in paths:
            try:
                zips.extract(path, dataset_root)
            except:
                pass
        # Verify extracted files
        assert self._check(checklist[len(urls) :], self.root)
