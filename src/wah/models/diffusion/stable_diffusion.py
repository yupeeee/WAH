...
from .load import _load_scheduler
...
model_path_dict = {
    ...
    "2.1-base": "Manojb/stable-diffusion-2-1-base",
}
...
def _load_pipe(
    ...
    **kwargs,
    ...
    assert version in model_path_dict, f"Unsupported version: {version}"
...
class StableDiffusion:
    ...
    # Load pipeline
    pipe = wah.models.StableDiffusion(
        version="1.4",
        scheduler="DDIM",
    )
    pipe.to("cuda")
    ...
    _model_path_dict = model_path_dict
    ...
    def supported_versions(self) -> List[str]:
        return list(self._model_path_dict.keys())
    ...
    def verbose(
        ...
        self._pipe.set_progress_bar_config(disable=not _verbose)
    ...
    def alphas_cumprod(
        ...
        for step in range(0, num_inference_steps):
            t = self._pipe.scheduler.timesteps[step]
            alphas_cumprod.append(self._pipe.scheduler.alphas_cumprod[t.item()])

        return torch.stack(alphas_cumprod, dim=0)  # (num_inference_steps, )

    def reset(self) -> "StableDiffusion":
        self._pipe._guidance_scale = 7.5
        self._pipe._guidance_rescale = 0.0
        self._pipe._clip_skip = None
        self._pipe._cross_attention_kwargs = None
    ...
    def init(
        ...
        if not height or not width:
            height = (
                self._pipe.unet.config.sample_size
                if self._pipe._is_unet_config_sample_size_int
                else self._pipe.unet.config.sample_size[0]
            )
            width = (
                self._pipe.unet.config.sample_size
                if self._pipe._is_unet_config_sample_size_int
                else self._pipe.unet.config.sample_size[1]
            )
            height, width = (
                height * self._pipe.vae_scale_factor,
                width * self._pipe.vae_scale_factor,
            )
        # to deal with lora scaling and other possible forward hooks

        # 1. Check inputs. Raise error if not correct
        self._pipe.check_inputs(
            prompt="",  # dummy prompt
            height=height,
            width=width,
            callback_steps=None,
        ...
        self._pipe._guidance_rescale = guidance_rescale
        self._pipe._clip_skip = clip_skip
        self._pipe._cross_attention_kwargs = cross_attention_kwargs
    ...
    def encode_prompt(
        ...
        lora_scale = (
            self._pipe.cross_attention_kwargs.get("scale", None)
            if self._pipe.cross_attention_kwargs is not None
            else None
        )
        ...
        prompt_embeds, negative_prompt_embeds = self._pipe.encode_prompt(
            ...
            clip_skip=self._pipe.clip_skip,
        )
    ...
    def encode_image(
        ...
        clip_input = self._pipe.feature_extractor(
    ...
    def encode(
        ...
        if isinstance(images, Image):
            images = [images]
        images = self._pipe.image_processor.preprocess(
            image=images,
            height=self._pipe.unet.config.sample_size * 8,
            width=self._pipe.unet.config.sample_size * 8,
        ).to(self.device)
        latents = self._pipe.vae.encode(images).latent_dist.mode()
    ...
    def decode(
        ...
        decoded = self._pipe.vae.decode(
            latents.to(self.device) / self._pipe.vae.config.scaling_factor,
            return_dict=False,
            generator=self._generator,
        )[0]
        pil_images = self._pipe.image_processor.postprocess(
            decoded,
            output_type="pil",
    ...
    def prepare_embeds(
        ...
        if self._pipe.do_classifier_free_guidance:
            prompt_embeds = torch.cat([negative_prompt_embeds, prompt_embeds])

        if ip_adapter_image is not None or ip_adapter_image_embeds is not None:
            image_embeds = self._pipe.prepare_ip_adapter_image_embeds(
                ip_adapter_image,
    ...
    def prepare_timesteps(
        ...
        num_warmup_steps = (
            len(timesteps) - num_inference_steps * self._pipe.scheduler.order
        )
    ...
    def prepare_latents(
        ...
        if self._pipe.unet.config.time_cond_proj_dim is not None:
            guidance_scale_tensor = torch.tensor(self._pipe.guidance_scale - 1).repeat(
                batch_size * self._num_images_per_prompt
            )
            timestep_cond = self._pipe.get_guidance_scale_embedding(
                guidance_scale_tensor,
                embedding_dim=self._pipe.unet.config.time_cond_proj_dim,
    ...
    def predict_noise(
        ...
        if hasattr(self._pipe.scheduler, "scale_model_input"):
            latent_model_input = self._pipe.scheduler.scale_model_input(
        ...
        with flag:
            noise_pred = self._pipe.unet(
                ...
                cross_attention_kwargs=self._pipe._cross_attention_kwargs,
    ...
    def perform_guidance(
        ...
        if guidance_scale is None:
            guidance_scale = self._pipe.guidance_scale
        ...
        if self._pipe.do_classifier_free_guidance and self._pipe.guidance_rescale > 0.0:
            # Based on 3.4. in https://huggingface.co/papers/2305.08891
            noise_pred = rescale_noise_cfg(
                noise_pred,
                noise_pred_text,
                guidance_rescale=self._pipe.guidance_rescale,
            )
    ...
    def step(
        ...
        latents = self._pipe.scheduler.step(
    ...
    def generate(
        ...
        if self._pipe.do_classifier_free_guidance:
            ...
            noise_preds = [
                noise_pred
                for noise_pred in torch.stack(noise_preds, dim=0).transpose(0, 1)
            ]
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # 7. Denoising loop
        with self._pipe.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if callback_on_step_end is not None:
                    ...
                    callback_outputs = callback_on_step_end(
                        self._pipe, i, t, callback_kwargs
                    )
                ...
                if i == len(timesteps) - 1 or (
                    ...
                    and (i + 1) % self._pipe.scheduler.order == 0
                ):
                    progress_bar.update()
        ...
        if not output_type == "latent":
            image = self._pipe.vae.decode(
                latents / self._pipe.vae.config.scaling_factor,
                return_dict=False,
                generator=self._generator,
            )[0]
            if self._pipe.safety_checker is not None:
                image, has_nsfw_concept = self._pipe.run_safety_checker(
