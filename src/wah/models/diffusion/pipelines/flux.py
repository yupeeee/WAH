...
class FluxPipeline(
    ...
    def __call__(
        ...
        if do_true_cfg:
            ...
            self._negative_text_ids = negative_text_ids
        self._image_embeds = image_embeds
        self._negative_image_embeds = negative_image_embeds
        self._guidance = guidance
        self._latent_image_ids = latent_image_ids
        self._do_true_cfg = do_true_cfg
...
from .utils import InitState, _detach_clone, _get_generator, _preprocess_prompt
...
def _init(
    ...
    **kwargs,
    ...
    prompt = _preprocess_prompt(prompt)
...
def _reset(
    ...
    pipe._width = ...
    pipe._prompt_embeds = ...
    pipe._pooled_prompt_embeds = ...
    pipe._text_ids = ...
    if pipe._do_true_cfg:
        pipe._negative_prompt_embeds = ...
        pipe._negative_pooled_prompt_embeds = ...
        pipe._negative_text_ids = ...
    pipe._image_embeds = ...
    pipe._negative_image_embeds = ...
    pipe._guidance = ...
    pipe._latent_image_ids = ...
    pipe._do_true_cfg = ...
    pipe._true_cfg_scale = ...
...
def _predict_noise(
    ...
    prompt_embeds = pipe._prompt_embeds
    pooled_prompt_embeds = pipe._pooled_prompt_embeds
    text_ids = pipe._text_ids
    do_true_cfg = pipe._do_true_cfg
    if do_true_cfg:
        negative_prompt_embeds = pipe._negative_prompt_embeds
        negative_pooled_prompt_embeds = pipe._negative_pooled_prompt_embeds
        negative_text_ids = pipe._negative_text_ids
    image_embeds = pipe._image_embeds
    negative_image_embeds = pipe._negative_image_embeds
    guidance = pipe._guidance
    latent_image_ids = pipe._latent_image_ids
    ...
    if do_true_cfg:
        if negative_image_embeds is not None:
            pipe._joint_attention_kwargs["ip_adapter_image_embeds"] = (
        ...
        with pipe.transformer.cache_context("uncond"):
            ...
            noise_pred = torch.cat([neg_noise_pred, noise_pred], dim=0)

    return _detach_clone(noise_pred)
...
def _perform_guidance(
    ...
    do_true_cfg = pipe._do_true_cfg
    true_cfg_scale = pipe._true_cfg_scale
...
def _rearrange_latents(
    ...
    latents: list[
        torch.Tensor
    ],  # List[Tensor(batch_size, C, H, W) * num_inference_steps]
) -> list[torch.Tensor]:
    latents = torch.stack(
        latents, dim=1
...
def _rearrange_noise_preds(
    ...
    noise_preds: list[
        torch.Tensor
    ],  # List[Tensor(batch_size(* 2), *D) * num_inference_steps]
) -> list[torch.Tensor | tuple[torch.Tensor, torch.Tensor]]:
    do_true_cfg = pipe._do_true_cfg

    noise_preds = torch.stack(
        noise_preds, dim=1
    ).cpu()  # Tensor(batch_size(* 2), num_inference_steps, *D)
    if do_true_cfg:
