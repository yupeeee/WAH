...
def save_in_csv(
    ...
    **kwargs,
    ...
    """
    assert (
        os.path.splitext(path)[1].lower() == ".csv"
    ), f"File extension must be .csv, got {os.path.splitext(path)[1].lower()}"
    for k, v in d.items():
...
def save_in_yaml(
    ...
    **kwargs,
    ...
    """
    assert (
        os.path.splitext(path)[1].lower() == ".yaml"
    ), f"File extension must be .yaml, got {os.path.splitext(path)[1].lower()}"
