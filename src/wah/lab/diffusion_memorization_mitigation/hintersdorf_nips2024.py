...
def hintersdorf_nips2024_single_prompt(
    prompt: str,
    pipe,
    sdv1_4_means: List[Tensor],
    sdv1_4_stds: List[Tensor],
    seed: Optional[Union[int, List[int]]] = None,
    verbose: bool = False,
) -> List[Image]:
...
def load_sdv1_4_stats() -> Tuple[List[Tensor], List[Tensor]]:
    ...
    sdv1_4_means, sdv1_4_stds = _tensor.load_from_url(
        sdv1_4_stats_url,
        use_jit=False,
    )
...
def hintersdorf_nips2024(
    ...
    for p in prompt:
        images.extend(
            hintersdorf_nips2024_single_prompt(
                p,
                pipe,
                sdv1_4_means,
                sdv1_4_stds,
