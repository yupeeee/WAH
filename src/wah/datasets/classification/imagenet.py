...
class ImageNet(ClassificationDataset):
    ...
    >>> dataset.name  # Get name of the dataset
    'ImageNet'
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if download:
            ...
            if not _lists.eq(
                _path.ls(data_root, fext="dir", sort=True, absolute=False),
                _lists.sort(list(set(_ilsvrc2012_val_meta))),
            ):
                if split == "train":
                    for wnid in tqdm.tqdm(
                        set(_ilsvrc2012_val_meta),
                        desc="Extracting ImageNet dataset (train)",
                        total=1000,
                    ):
                        fpath = os.path.join(data_root, f"{wnid}.tar")
                        _zips.extract(fpath, save_dir=os.path.join(data_root, wnid))
                    ...
                    for wnid, image_path in tqdm.tqdm(
                        zip(
                            _ilsvrc2012_val_meta,
                            _path.ls(data_root, fext=".JPEG", sort=True, absolute=True),
                        ),
                        desc="Extracting ImageNet dataset (val)",
                        total=len(_ilsvrc2012_val_meta),
                    ):
                        shutil.move(
                            image_path,
